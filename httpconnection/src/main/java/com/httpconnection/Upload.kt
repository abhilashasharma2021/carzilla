    ...
    fun executorString(response: com.e.mylibrary.Response) {
        if (checkAuth == 2) {
            Fuel.upload("")
                ...
                .responseString { requestL, responseL, resultL ->
                    if (responseL.statusCode == 200) {
                        response.onResponse(resultL.get())

                    } else {
                        val executorException = ExecutorException(
                            1003, "${responseL.responseMessage}",
                            "${resultL.component2()?.localizedMessage}"
                        )
            ...
            Fuel.upload(pathURL!!, parameters = getBodyParameter.toList())
                .plus(dataParts)
                .responseString { request, responseL, resultL ->
                    if (responseL.statusCode == 200) {
                        response.onResponse(resultL.get())
